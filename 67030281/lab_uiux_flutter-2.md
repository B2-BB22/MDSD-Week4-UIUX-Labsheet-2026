...
```
คำตอบ: Material 3 เปลี่ยนจากระบบสีตายตัว มาเป็นระบบ Dynamic Color ที่สร้างจาก "Seed Color" เพียงสีเดียว แล้ว generate เป็นชุดสีที่สัมพันธ์กันทั้งหมดโดยอัตโนมัติ (ผ่านเครื่องมือ Material Theme Builder) นอกจากนี้ยังมีแนวคิด "คู่สี" ที่ชัดเจนกว่าเดิม คือทุกสีพื้นหลัง (เช่น Primary, Secondary, Tertiary) จะมีสีคู่กันเสมอ คือ "On" สี (เช่น onPrimary) สำหรับข้อความ/ไอคอนที่วางทับ เพื่อรับประกัน contrast ตามหลัก accessibility นอกจากนี้ยังเพิ่มระดับ Container (PrimaryContainer, SecondaryContainer) ให้มีสีเดียวกันแต่ต่างระดับ ความเข้ม ใช้แยกความสำคัญขององค์ประกอบ (เช่น ปุ่มหลักใช้ Primary ส่วนพื้นหลังการ์ดรองใช้ PrimaryContainer) และมี Surface หลายระดับ สำหรับสร้างมิติความลึกแทนการใช้ elevation shadow เป็นหลักแบบ Material 2
```
...
ต้องสลับไปใช้ library อื่น แทน และไอคอน เฉพาะของแต่ละสินค้าที่ตั้งใจให้ต่างกัน (eco, shopping_basket, lightbulb, local_florist) ต้องมาแมปเองตอนเขียนโค้ด เพราะ Figma component ไม่ได้ export ข้อมูลนี้มาให้อัตโนมัติ 
...
```
คำตอบ: โค้ดที่ AI สร้าง (ทั้งจาก Google AI Studio และรอบหลัง) ไม่สมบูรณ์ 100% แม้จะดูเหมือนใช้งานได้ในตอนแรก พบปัญหาที่ต้องแก้ ก่อน compile ผ่านคือ typo ชื่อ class ผิด (CrossAlignment แทนที่จะเป็น CrossAxisAlignment) ซึ่งเกิดซ้ำถึง 2 ครั้งในคนละไฟล์ แสดงว่าเป็นจุดอ่อน ที่ AI มักผิดพลาดซ้ำๆ นอกจากนี้ยังมีจุดที่ควรปรับปรุงเพิ่มแม้ compile ผ่าน เช่น ไม่มี error handling เมื่อโหลดรูปจาก NetworkImage ไม่สำเร็จ, ไม่มี Semantics/ Tooltip สำหรับ Accessibility, และโครงสร้างสถาปัตยกรรมบางเวอร์ชัน ไม่สอดคล้องกับที่ต้องการ
```
...
```
คำตอบ: 
1) เพิ่ม error handling ที่ครบถ้วนกว่านี้ เช่น การโหลดรูปสินค้า/ avatar ไม่สำเร็จ ควร fallback เป็น placeholder หรือตัวอักษรย่อเสมอ ไม่ใช่ปล่อยให้ error เงียบๆ 
...
4) ทดสอบ Dark Mode และ Contrast ให้ครบทุกหน้าจอก่อนใช้งานจริง เพราะสีที่ดูดีใน Light Mode อาจมีปัญหา contrast ต่ำใน Dark Mode 
   
5) เพิ่ม State Management ที่เหมาะสมกับขนาดแอป (เช่น Provider, Riverpod, Bloc) แทนการใช้ setState() ธรรมดา เพราะแอปจริงจะมีข้อมูล ที่ซับซ้อนและใช้ร่วมกันหลายหน้ามากขึ้น 
